...
void endProg(void)
{
   ...
   if(srv) delete srv;
   closelog();
   printf("Program of %s was terminated!\n",DAEMON_NAME );
}
...
int main(int argc, char *argv[])
{
    ...
    if(argc>1){
      if(!strcmp(argv[1],"stop")){
        ...
        printf("Usage %s [run | stop | debug | console   tm | tv |tg]\n",argv[0]);
        exit(EXIT_FAILURE);
      }
    }
    else{
...
// Daemonize if not debug mode
  ...
  srv=new TTDBehav();
  if(srv) srv->start(QThread::NormalPriority);
  else {
    printf("Can't create TTDBehav object. Program was terminated.\n");
    syslog( LOG_ERR,"Can't create TTDBehav object. Program was terminated.");
    exit(EXIT_FAILURE);
  }
  a.exec();
  return 0;
